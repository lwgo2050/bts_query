...
$(function() {
    ...
    $button.click(function() {
        dataArr = []; // 清除缓存数据
        var symbol = $('#search_asset').val().split(":");
        if (symbol.length == 2) {
            symbol_base = symbol[0];
            symbol_quote = symbol[1];
        }
        else {
            alert("请检查输入的交易对是否正确");
            return;
        ...
        while (startDate>endDate) {
            ...
            // console.log(startDate);
            Api.instance().db_api().exec( "get_trade_history", [symbol_base, symbol_quote,toUnixStamp(start),toUnixStamp(end),100])
            .then(response => {
        ...
        console.log(last);
        console.log(endDate);
        Api.instance().db_api().exec( "get_trade_history", [symbol_base, symbol_quote,toUnixStamp(last),toUnixStamp(endDate),100])
        .then(response => {
            console.log(response);
            var account_set = new Set();
            for (var i = 0; i < response.length; i++) {
...
function loadData(data){
    ...
    queryResultStr ='<span style="color:#FF0000">'+ symbol_base+ ":"+symbol_quote+"查询成功</span>";
 // 隐藏查询进度条
